...
- **腾讯视频**：无 cookie 时 VIP/付费内容只能试看一小段；免费内容不受影响。
  注意：少数头部独播内容（如斗破苍穹年番等热门动漫，接口标记 drm=1）全集
  只有 Widevine 加密流，明文通道一律只给试看——这类内容任何下载工具都拿
  不到全集，不属于 bug。另外 VIP 接口的会话时间戳（约 1.5 小时过期）由工具
  自动续期，不用因此频繁重导 cookie；真正需要重导的是登录态本身失效
  （vuserid/vusession 被腾讯轮换，通常更久）。
